...
import { PatientSection } from "@/components/patient/PatientAppShell";

interface InteractiveDentalChatProps {
	user: User | null;
	triggerBooking?: 'low' | 'medium' | 'high' | 'emergency' | false;
	onBookingTriggered?: () => void;
	seedFromBookNow?: boolean;
	onSeedConsumed?: () => void;
	onNavigateSection?: (section: PatientSection) => void;
}
...
export const InteractiveDentalChat = ({ 
	...
	// Seed Dentibot greeting when opened via Book now, unless user has already started
	useEffect(() => {
		const started = (() => {
			try { return localStorage.getItem('pd_assistant_started') === 'true'; } catch { return false; }
		})();
		const prefillActive = (() => {
			try { return localStorage.getItem('pd_assistant_prefill_active') === 'true'; } catch { return false; }
		})();

		if ((seedFromBookNow || prefillActive) && !started && messages.length === 0) {
			const greeting = "Hello, I’m Dentibot. I’m here to assist you with your appointments, prescriptions, payments, and care questions. Would you like to book an appointment now?";
			...
			try { localStorage.setItem('pd_assistant_prefill_active', 'true'); } catch {}
			onSeedConsumed?.();
		}
	}, [seedFromBookNow]);
	...
	const initializeChat = () => {
		// Skip default welcome if prefill is active
		const prefillActive = (() => {
			try { return localStorage.getItem('pd_assistant_prefill_active') === 'true'; } catch { return false; }
		})();
...
You'll receive a confirmation email shortly. If you need to reschedule or cancel, just ask me! 😊`;
	...
	const handleLanguageChange = (lang: string) => {
		changeLanguage(lang);
		addBotMessage(`Language changed to ${lang.toUpperCase()}`);
	};

	const handleQuickSuggestion = (action: 'book' | 'view' | 'pay' | 'medication') => {
		try { localStorage.setItem('pd_assistant_started', 'true'); localStorage.removeItem('pd_assistant_prefill_active'); } catch {}
		...
		switch (action) {
			...
			case 'view':
				...
				break;
			case 'pay':
				addBotMessage("Opening Payments. You can also find this under the Payments tab.");
				onNavigateSection?.('payments');
				break;
			case 'medication':
				addBotMessage("Sure — what medication would you like to ask about?");
				break;
		}
	};
					...
					{showQuickSuggestions ? (
						<div className="pt-2">
							<div className="flex flex-wrap gap-2">
								<Button variant="outline" size="sm" className="rounded-full" onClick={() => handleQuickSuggestion('book')}>
									Book an appointment
								</Button>
								<Button variant="outline" size="sm" className="rounded-full" onClick={() => handleQuickSuggestion('view')}>
									View upcoming appointment
								</Button>
								<Button variant="outline" size="sm" className="rounded-full" onClick={() => handleQuickSuggestion('pay')}>
									Pay balance
								</Button>
								<Button variant="outline" size="sm" className="rounded-full" onClick={() => handleQuickSuggestion('medication')}>
