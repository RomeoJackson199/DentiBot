...
export const Header = ({ user }: HeaderProps) => {
  ...
  const navigation = [
    { name: 'Emergency Triage', href: '/emergency-triage', icon: Activity },
        ...
        <div className="flex items-center justify-between py-4 lg:py-6">
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {navigation.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="flex items-center space-x-2 text-dental-muted-foreground hover:text-dental-primary transition-colors duration-300 font-medium"
              >
          ...
          <div className="flex items-center space-x-4">
            {!user ? (
              <>
                ...
                <div className="block sm:hidden">
                  <Button 
                    size="sm" 
                    variant="outline"
                    className="bg-white/10 backdrop-blur-sm border-white/20 text-dental-primary"
                  >
                    Sign In
                  </Button>
                </div>
              ...
              <Button 
                variant="outline"
                className="bg-white/10 backdrop-blur-sm border-white/20 text-dental-primary hover:bg-white/20"
                asChild
              >
                <a href="/dashboard">
            ...
            <nav className="space-y-4">
              {navigation.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="flex items-center space-x-3 text-dental-muted-foreground hover:text-dental-primary transition-colors duration-300 font-medium py-2"
