      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <Card 
              key={index} 
              className="glass-card hover:shadow-elegant transition-all duration-300 group cursor-pointer border-0"
            >
              <CardContent className="p-8">
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  <Badge 
                    variant="secondary" 
                    className="text-xs bg-white/20 text-dental-primary border-0"
                  >
                    {feature.badge}
                  </Badge>
                </div>
        ...
        <div className="text-center mt-16">
          <div className="glass-card inline-flex items-center gap-4 p-6 rounded-2xl">
            <Heart className="w-6 h-6 text-red-500" />
            <span className="text-lg font-medium">
              Join thousands of practices already using our platform
            </span>
          </div>
        </div>
      </div>
