...
export function ModernHeroSection({ onBookAppointment, onStartTriage, onOpenAIChat, variant = "default" }: ModernHeroSectionProps) {
          ...
          {isCtaFocused ? (
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 max-w-md border border-white/10">
              <div className="flex items-center gap-3 text-sm text-white/90">
                ...
                {language === 'fr'
                  ? 'Des milliers de patients prennent soin de leur sourire avec DentiBot'
                  : language === 'nl'
                  ? 'Duizenden patiënten verzorgen hun glimlach met DentiBot'
                  : 'Thousands of patients trust DentiBot with their smile'}
              </div>
            </div>
          ) : (
