...
CREATE TABLE public.dentist_capacity_settings (
  ...
  emergency_slot_release_hours INTEGER DEFAULT 24,
...
CREATE TABLE public.slot_recommendations (
  ...
  dentist_id UUID NOT NULL REFERENCES public.dentists(id) ON DELETE CASCADE,
  business_id UUID NOT NULL REFERENCES public.businesses(id) ON DELETE CASCADE,
  recommended_slots JSONB NOT NULL,
...
CREATE POLICY "Business members can manage appointment types" ON public.appointment_types
  FOR ALL USING (
    business_id = get_current_business_id() AND
    is_business_member((SELECT id FROM public.profiles WHERE user_id = auth.uid()), business_id)
  );
...
CREATE POLICY "Business members can manage capacity settings" ON public.dentist_capacity_settings
  FOR ALL USING (
    business_id = get_current_business_id() AND
    is_business_member((SELECT id FROM public.profiles WHERE user_id = auth.uid()), business_id)
  );
...
CREATE POLICY "System can create recommendations" ON public.slot_recommendations
...
CREATE POLICY "System can create reschedule suggestions" ON public.reschedule_suggestions
  FOR INSERT WITH CHECK (true);
